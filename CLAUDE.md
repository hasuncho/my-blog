...
- 모바일에서도 사용할 수 있어야 한다.
- 모든 웹앱은 블로그의 색상 팔레트를 따른다. style.css(css/variables.css)의 CSS 변수(--color-* 등)를 참조해서 스타일링할 것.
- 웹앱에는 사용법 안내 문구를 반드시 포함한다 (조작 방법 등을 화면에 표시).
